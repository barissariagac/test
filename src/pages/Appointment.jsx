...
export default function Appointment({ onBooked }) {
  const me = currentUser()
  const today = new Date().toISOString().split('T')[0]
  const [selectedDate, setSelectedDate] = useState(today)
  const [selectedTime, setSelectedTime] = useState(null)
  const [formData, setFormData] = useState({ name: me?.name || '', email: me?.email || '', message:'' })
  ...
  const handleSubmit = (e) => {
    ...
    setIsLoading(true)
    setTimeout(() => {
      const created = addAppointment({ date: selectedDate, time: selectedTime, ...formData })
      if (typeof onBooked === 'function') { try { onBooked(created) } catch {} }
      setIsLoading(false)
      setIsSubmitted(true)
    }, 700)
  }
    ...
    <div className="">
      ...
      <form onSubmit={handleSubmit} className="">
        ...
        <button type="submit" disabled={!selectedTime||isLoading}
          ...
          <Calendar size={18}/> Randevu Talebi Gönder
        </button>
      </form>
    </div>
  )
